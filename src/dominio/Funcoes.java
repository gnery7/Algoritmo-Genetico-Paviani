...
public class Funcoes {
    ...
    private double f12(double x[]) {
        ...
        double produtorio = 0;
        for (int i = 0; i < x.length; i++) {
            somatorio += Math.pow(Math.log(x[i] - 2), 2) + Math.pow(Math.log(10 - x[i]), 2);
            if (i == 0) {
                produtorio = x[i];
            } else {
                produtorio *= x[i];
        ...
        resultado = somatorio - Math.pow(produtorio, 0.2);
        return somatorio - Math.pow(produtorio, 0.2);
    }
